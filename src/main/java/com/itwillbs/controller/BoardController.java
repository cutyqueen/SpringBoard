...
public class BoardController {
	...
	public void readGET(@RequestParam("bno") int bno, 
		...
		if((boolean) session.getAttribute("viewcntCheck")) {
			// 서비스 - bno에 해당하는 글 조회수 1증가(페이지 호출당 1번씩 증가/read페이지 새로고침시 증가X)
...
//		model.addAllAttributes(resultVO); => 이름 설정 안하면 "boardVO" 라는 이름으로 호출됨
	 ...
	 * 2) 페이지 하단에 '페이지 블럭'을 생성
	 * 3) 본문/수정/삭제 등... -> 처리후 다시 리스트로 이동 시 기존정보 유지
	 * 
	 *  a태그 : 네이버쇼핑 / 유사한 코드의 반복적인 동작 수행
	 *	  		검색엔진 노출이 쉬움
	 *  
	 *  form태그  : 쿠팡 / input 태그를 사용해서 처리
	 *  			데이터처리(빠른처리)
	 *  
	 *  - 하단 페이징블럭
	 *  ex) 글 총 122개 / 페이지당 10개씩 출력
	 *      1페이지  : 시작페이지 번호 : 1  | 끝페이지 번호 : 10  | 이전 : N | 다음 : Y
	 *      7페이지  : 시작페이지 번호 : 1  | 끝페이지 번호 : 10  | 이전 : N | 다음 : Y 
	 *      12페이지 : 시작페이지 번호 : 11 | 끝페이지 번호 : 13  | 이전 : Y | 다음 : N 
	 *        
	 *  
	 */
	...
	// http://localhost:8088/board/listPage?page=1&pageSize=10
	// 게시판 리스트 페이징처리- GET
