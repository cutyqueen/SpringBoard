			...
			<div class="form-group">
				<label for="exampleInputEmail1">작성자</label>
				 <input type="text" class="form-control" id="exampleInputEmail1" name="writer"
				        value ="${resultVO.writer }" readonly>
			</div>
			<div class="form-group">
...
<script type="text/javascript">
	$(document).ready(function() {
		// 목록버튼 클릭시 목록으로 페이지 이동
		$(".btn-primary").click(function() { //btn-primary를 클릭했을 때
			location.href='/board/listAll';
		});
	
		var formObj=$('form[role="form"]'); // role 속성이 "form"으로 설정된 <form> 요소를 선택
		console.log(formObj);
		
		// 수정버튼 클릭시, 글번호 정보를 가지고 submit
		// 이동하는 페이지주소 변경, 전달방식 변경POST -> GET(처음엔 기존정보 보여줘야하니까
